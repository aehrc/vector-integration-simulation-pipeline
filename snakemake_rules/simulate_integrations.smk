...
rule simulate_integrations:
	...
	resources:
		mem_mb= lambda wildcards, attempt, input: attempt * 5 * int((path.getsize(input.host)/1000000)) + get_parameter(wildcards, 'int_num') * 100 + get_parameter(wildcards, 'epi_num') * 100,
		time = "24:00:00",
		nodes = 1
	params:
